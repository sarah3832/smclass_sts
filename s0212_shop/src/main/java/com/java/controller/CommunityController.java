...
public class CommunityController {
	...
	// 포토리뷰 공공데이터 불러오기
	@ResponseBody  // 데이터로 전송 (@RequestController 와 동일)
	@PostMapping("/community/api_data")
	public String api_data() throws Exception {
		//api 호출
	    String service_key = "C2duPwoVyzL4a65JGcp9k28ZiHIuugwI%2BPj1H1uIhqNZVM5fv1yho7%2BRyFN%2FxU7mYmWhPyxAqbyNxfqQEJi34w%3D%3D";
	...
	public String api_comment() throws Exception {
        ...
        urlBuilder.append("&" + URLEncoder.encode("pageNo","UTF-8") + "=" + URLEncoder.encode(page, "UTF-8")); /*페이지번호*/
        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
        urlBuilder.append("&" + URLEncoder.encode("from","UTF-8") + "=" + URLEncoder.encode("1982-01-01", "UTF-8")); /*요청자료형식(XML/JSON)*/
        urlBuilder.append("&" + URLEncoder.encode("until","UTF-8") + "=" + URLEncoder.encode("1982-01-01", "UTF-8")); /*요청자료형식(XML/JSON)*/
